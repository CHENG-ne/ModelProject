...
<script type="text/javascript">
	$(function(){
		...
		$("#loginInfoDisplay").typeahead({
			...
			items:5,//下拉框显示的数据条数
			minLength:3,//最小3个字符才发送请求查询自动补全数据
			displayText:function(item){//下拉框默认显示name字段,更改为显示username字段
				return item.username;
			},
			autoSelect:true
		});
		$("#loginInfoDisplay").change(function(){
			var current=$(this).typeahead("getActive");
			$("#applierId").val(current.id);
		});
	});
	</script>
</head>
					...
					<form id="searchForm" class="form-inline" method="post"
						...
						<div class="form-group">
							<label>关键字</label> <input class="form-control"
								style="width: 130px;" type="text" name="keyword"
								id="keyword" value="${(qo.keyword)!''}" />
						</div>
						<div class="form-group">
							...
							<div class="col-sm-6">
								<div class="dropdown" id="autocomplate">
									<input type="text" class="form-control" id="loginInfoDisplay"
										autocomplete="off" /> <input type="hidden" name="applierId" id="applierId"/>
								</div>
							</div>
						</div>
						<div class="form-group">

package com.cheng.normal.business.util;

import java.math.BigDecimal;
import java.math.RoundingMode;

import com.cheng.normal.base.util.BidConst;
...
public class CalculatetUtil {
		...
		if (returnType == BidConst.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			...
			if (monthes2Return == 1) {
				...
				if (monthIndex < monthes2Return) {
					monthlyInterest = DecimalFormatUtil.amountFormat((temp1.subtract(monthToReturnMoney)).multiply(temp4).add(monthToReturnMoney)
	...
	// return
	// DecimalFormatUtil.formatBigDecimal(amount.multiply(BidConst.RECHARGE_FEE_RATE),
	...
	public static BigDecimal calInterestManagerCharge(BigDecimal interest) {
		return DecimalFormatUtil.formatBigDecimal(interest.multiply(INTEREST_MANAGER_CHARGE_RATE), BidConst.STORE_SCALE);
	}
	...
	public static BigDecimal calAccountManagementCharge(BigDecimal bidRequestAmount) {
		BigDecimal accountManagementCharge = DecimalFormatUtil.formatBigDecimal(bidRequestAmount.multiply(ACCOUNT_MANAGER_CHARGE_RATE), BidConst.CAL_SCALE);
